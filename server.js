...
app.post('/api/sales', async (req, res) => {
  try {
    ...
    const saleId = saleResult.rows[0]?.id || Number(saleResult.lastInsertRowid);
...
app.post('/api/purchases', async (req, res) => {
  try {
    ...
    const purchaseId = purchaseResult.rows[0]?.id || Number(purchaseResult.lastInsertRowid);
    for (const item of items) {
      await db.execute({
...
initDatabase().then(() => {
  app.listen(PORT, () => {
    console.log(`KasirKu Server berjalan di http://localhost:${PORT}`);
  });
});
